...
IMPLEMENT_DYNAMIC(CSplitterControl, CStatic)
/////////////////////////////////////////////////////////////////////////////
// CSplitterControl
...
void CSplitterControl::OnMouseMove(UINT nFlags, CPoint point) 
{
	if (m_bIsPressed)
	{
		...
		if (m_nY>rc.bottom-10) m_nY = rc.bottom-10;
		if (m_nX>rc.right-10) m_nX = rc.right-10;

		if (m_pLeft)
		{
			CRect r1;
			m_pLeft->GetWindowRect(&r1);
			if (m_nX<=r1.right) m_nX = r1.right + 1;
		}

		if (m_pRight) {
			CRect r2;
			m_pRight->GetWindowRect(&r2);
			if (m_nX>=r2.left) m_nX = r2.left - 1;
		}
...
BOOL CSplitterControl::PreCreateWindow(CREATESTRUCT& cs)
{
...
//	this->MoveWindow(rc,0);
	...
	// 添加事件响应风格
	ModifyStyle(WS_BORDER, SS_NOTIFY, 0);
	
	CStatic::PreSubclassWindow();
	
}

void CSplitterControl::OnMyMouseMove( CPoint point )
{
	if (m_bIsPressed)
	{
