...
BOOL CRGBImageList::Create( UINT idBitmap, UINT nWidth )
{
...
BOOL CRGBImageList::Draw( CDC* pDC, int nImage, POINT pt )
{
	return  pDC->BitBlt( pt.x, pt.y, m_width, m_height, &m_dcMem, nImage*m_width, 0, SRCCOPY );
}

CSize CRGBImageList::GetImageDimension()
{
	CSize s(m_width,m_bmpInfo.bmHeight); return s;
}
